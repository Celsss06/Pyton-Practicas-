...
miEtiqueta1 = Label(root, text="Bienvenido al registro de alumnos", bg="MediumPurple1", font=("Chamberi Super Display", 13), fg="gray1")
miEtiqueta1.pack()


#Se crea el login, aqui falla xd
def iniciar_sesion():
    usuario = nombre_usuario.get()
    password = contrasena_usuario.get()

    if usuario == "celss03" and password == "06062023":
...
label.config (fg= "Black", bg="MediumPurple1", font=("Chamberi Super Display", 13))
label.pack (padx=0)
...
salir = tk.Button(root, text="Salir",  font= ("Times New Roman", 11), command=root.quit)
salir.pack()
iniciar_sesion = Button(root, text="Iniciar sesion", bg="white", font= ("Times New Roman", 11), fg="Black", command=funcion)
iniciar_sesion.pack()


# Crear un widget de etiqueta para mostrar el resultado del inicio de sesión
resultado= tk.Label(root, bg="MediumPurple1")
...
Otraventana.state(newstate = "withdraw")
Otraventana.geometry("500x500")
Otraventana.title("Ventana 3")
miEtiqueta = Label(Otraventana, text="Bienvenido a la ventana 3", bg="#252850", font=("Times New Roman", 12), fg="yellow")
miEtiqueta.pack()
...
miEtiqueta.title("Ventana 2 ")
miEtiqueta.config(bg="MediumPurple4")
# Crear los campos de entrada para los datos del alumno
Label(miEtiqueta, text="Id:", bg="MediumPurple4").grid(row=0, column=0, padx=5, pady=5)
entrada_id = Entry(miEtiqueta)
entrada_id.grid(row=0, column=1, padx=5, pady=5)

Label(miEtiqueta, text="Nombre:",bg="MediumPurple4").grid(row=1, column=0, padx=5, pady=5)
entrada_nombre = Entry(miEtiqueta)
entrada_nombre.grid(row=1, column=1, padx=5, pady=5)

Label(miEtiqueta, text="Edad:",bg="MediumPurple4").grid(row=2, column=0, padx=5, pady=5)
entrada_edad = Entry(miEtiqueta)
entrada_edad.grid(row=2, column=1, padx=5, pady=5)

Label(miEtiqueta, text="Email:",bg="MediumPurple4").grid(row=3, column=0, padx=5, pady=5)
