#Las mejoras realizaas son que, se le agregó un inicio de secion que tiene nombre de usuario y contrseña especificos
...
def ventanacrud():
    ...
    def validate_name(text):
        return text.isalpha() and len(text) <= 15
    ...
    entrada_id = Entry(Etiqueta, validate="key", validatecommand=(Etiqueta.register(validate_id), '%P'))
    entrada_id.grid(row=0, column=1, padx=5, pady=5)

    Label(Etiqueta, text="Name:", bg="MediumPurple4").grid(row=1, column=0, padx=5, pady=5)
    entrada_nombre = Entry(Etiqueta, validate="key", validatecommand=(Etiqueta.register(validate_name), '%P'))
    ...
    entrada_edad = Entry(Etiqueta, validate="key", validatecommand=(Etiqueta.register(validate_age), '%P'))
    entrada_edad.grid(row=2, column=1, padx=5, pady=5)

    Label(Etiqueta, text="Carrera:", bg="MediumPurple4").grid(row=3, column=0, padx=5, pady=5)
    entrada_email = Entry(Etiqueta, validate="key", validatecommand=(Etiqueta.register(validate_name), '%P'))
    ...
    def agregar_alumno():
        ...
        agregar_alumnoDB(nombre, edad, email,id)
    ...
    cursor = db.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS alumnos (id INT  PRIMARY KEY, nombre VARCHAR(255), edad INT, email VARCHAR(255))")
    ...
    def agregar_alumnoDB(nombre, edad, email,id):
        try:
            cursor = db.cursor()
            cursor.execute("INSERT INTO alumnos (nombre, edad, email,id) VALUES (%s, %s, %s,%s)", (nombre, edad, email,id))
            db.commit()
