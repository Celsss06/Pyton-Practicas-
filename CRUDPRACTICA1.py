...
def ventanacrud():
    root.destroy()
    miEtiqueta = Tk()
    miEtiqueta.title("REGISTRO ")
    miEtiqueta.config(bg="MediumPurple4")
    miEtiqueta.geometry("300x300")
...
def insesion():
    if contrasena_usuario.get() == "06062022" and nombre_usuario.get() == "Renata":
            ventanacrud()
    else:
            messagebox.showinfo(message="La contraseña o el nombre son incorrectas :(", title="ERROR")
#SE CREA LA VENTANA
global root
root = Tk()
...
miEtiqueta1 = Label(root, text="Bienvenido al registro de alumnos", bg="MediumPurple1", font=("Times New Roman", 13), fg="gray1")
miEtiqueta1.pack()
...
salir.pack()
iniciar_sesion = Button(root, text="Iniciar sesion", bg="white", font= ("Times New Roman", 11), fg="Black", command=insesion)
